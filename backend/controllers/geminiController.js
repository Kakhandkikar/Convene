...
export async function generate(req, res) {
  try {
    ...
    let result;
    try {
      result = await callGemini(form);
    } catch (err) {
      // Surface Gemini timeout separately
      if (String(err.message || err).includes('timeout')) {
        return res.status(504).json({ error: `Scheduling service timeout: ${err.message}` });
      }
      throw err;
    }
    ...
    res.json({ organizerTime, clientTime, reasoning, meetingId: meeting.meetingId, _id: meeting._id, timings: { geminiMs: tGemini - t0, dbMs: tDb - tGemini } });
  } catch (e) {
